...
class GeneratorPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    IconData icon;
    if (appState.favoriteWords.contains(pair)) {
      icon = Icons.favorite;
    } else {
...
class FavoritesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var itemsList = favoriteWords.map((w) => Text(w.asLowerCase)).toList();

    return Column(
      children: [
        Padding(
          padding: const EdgeInsets.all(8),
          child: Text('You have ${itemsList.length} favorited words!'),
        ),
        Expanded(
